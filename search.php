...
<html lang="it">
    <head>
        ...
        <link href="endors/datatables.net-responsive-bs/css/responsive.bootstrap.min.css" rel="stylesheet">
                          ...
                          <tr>
                            ...
                            <tboydy>
                              ...
                              if(isset($_POST['Submit'])) {
                                  $ragione_sociale = $_POST['stabilimento'];
                                  $citta = $_POST['citta'];
                                  if(empty($ragione_sociale) && empty($citta) ) {

                                    $rs = mysqli_query($conn, "SELECT chalet_id, ragione_sociale,numero_concessione, telefono, CONCAT(indirizzo,' , ',cap,' - ',citta, ' (',provincia, ')') 
                                    ...
                                    FROM chalet ");
                                        while($row = mysqli_fetch_array($rs)) {
                                              ...
                                              <form action='book.php' class='search-property-1' method= 'POST'>
                                                <a href='book.php' class='btn btn-primary' role = 'button'><button class='btn btn-primary' type='submit' value = ".$row['chalet_id']." id= 'Submit' name= 'Submit'  class='form-control btn btn-primary'>PRENOTA</button></a> </td>
                                              </form>
                                              </tr>";
                                        }                                                    
                                  } else{  
                              
                                  //chalet_id	ragione_sociale	numero_concessione	telefono	cellulare	provincia	citta	logo	user_id
                                  ...
                                  FROM chalet 
                                  where ragione_sociale = '$ragione_sociale' or citta = '$citta' ");
                                  if (!$rs) {
                                    printf("Error: %s\n", mysqli_error($conn));
                                    exit();
                                  }
